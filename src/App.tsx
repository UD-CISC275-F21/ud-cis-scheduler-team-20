    ...
    const [courses, setCourses] = useState([
        {
            "rowData":[
                ...
                {id:1,Course: "CISC 210", Credit: "3",Name:"Introduction to Computer Science 2",Plan:"Take Care",sDegreeRequire:"CISC",Presuit:"CISC 108"},
                ...
                if(flag && DropObjIndex!=-1 && nowDragData && nowDragData.id!==""&& nowDragData.id!==null){
                    ...
                    if(DropObjIndex==-2) {
                        ...
                        if(arr[DropObjIndex]) {
                            const parseData = JSON.parse(JSON.stringify(nowDragData));
                            let flag2 = false;
                            for(let i = 0; i< arr[DropObjIndex].rowData.length ;i++) {
                                const item = arr[DropObjIndex].rowData[i];
                                if(nowDragData.id && item.id == nowDragData.id) {
                                    alert("The course already exists");
                                    DropObjIndex = -1;
                                    nowDragData = {} as Element;
                                    return;
